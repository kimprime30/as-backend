...
export const addPerson: RequestHandler = async (req, res) => {
  ...
  if (!body.success) {
    console.log('Erro de validação:', body.error); // Log de erro de validação
  ...
  if (newPerson) {
    console.log('Pessoa adicionada com sucesso:', newPerson); // Log de sucesso
    return res.status(201).json({ person: newPerson });
  } else {
    console.log('Erro ao adicionar pessoa.'); // Log de erro
    return res.json({ error: 'Ocorreu um erro' });
  }

  console.error('Erro inesperado:'); // Log de erro inesperado
  return res.json({ error: 'Ocorreu um erro' });
